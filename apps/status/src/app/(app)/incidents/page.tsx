...
import services, {
	type IncidentServiceFunctionReturnType,
...
export default async function IncidentsPage({
	...
	const incidents = await services.incident.listIncidents();
	...
	const incidentsByMonth = monthsToShow.map((date) => {
	...
	return (
		<div className="flex flex-col items-center justify-start gap-4 w-full h-full">
			<div className="w-full flex flex-col md:flex-row gap-4 items-center justify-center md:justify-between">
			...
			<div className="w-full h-full px-4">
				{incidentsByMonth.map(({ date, incidents }) => (
					<MonthIncidents
		...
		<div className="mt-11 dark:bg-[#191C24] border border-[#E2E4E9] dark:border-[#21242D] rounded-lg shadow-small overflow-hidden">
			...
			{incidents?.length === 0 ? (
				<div className="my-14 text-large text-center font-medium">
					...
					Henüz bir olay rapor edilmemiş.
				</div>
			) : (
				incidents?.map((incident) => (
					<React.Fragment key={incident.id}>
						<div className="px-7 py-3 bg-[#F5F5F7] dark:bg-[#21242D] flex justify-between  text-sm ">
							<div className="font-medium">
								{format(new Date(incident.created_at as Date), "dd MMMM yyyy", {
									locale: tr,
								})}
							</div>
							<div>1 Olay</div>
						</div>
						<IncidentComponent className="rounded-none" incident={incident} />
					</React.Fragment>
				))
			)}
		</div>
